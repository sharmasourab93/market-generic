...
from trade.technicals.indicators import GenericIndicator, PivotPoints, MovingAverages
...
PICKED_COLS = ["symbol", "pct_change", "cpr", "cpr_width", "EMA20", "EMA50",
                   "EMA200", "pe", "Expiry", "Max Put OI", "PCR at Max", "Max Call OI",
                   "PCR at Min", "Nearest Support", "Resistance at", "Straddle",
                   "Overall PCR", "Verdict", "Max Put & Call OI at"]
...
class SpotIndices(AllDataGenerics):
    ...
    def get_top_bottom(self, top_n: int = 200, n_value: int = 5):
        ...
        largest = data.nlargest(n_value, "pct_change")[["symbol",
                                                        "pct_change"]].to_dict(
            orient="records")
        smallest = data.nsmallest(n_value, "pct_change")[["symbol",
                                                        "pct_change"]].to_dict(
            orient="records")
        return {
            "top_n": largest,
    ...
    def comprehensive_report_as_dataframe(self) -> REPORT_FORMAT:
        ...
        index_data = pd.concat([index_data.reset_index(),
                                pivots, mas], axis=1).drop(["pivots","mas"],
                                                                axis=1)
        ...
        index_data = pd.concat([index_data, vix], axis=0).drop("adv-dec",
                                                               axis=1).fillna("")
        ...
        index_data = pd.merge(index_data, option_chain, on="symbol",
                            how="left").fillna("")
